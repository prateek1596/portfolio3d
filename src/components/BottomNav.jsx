...
export default function BottomNav({ active, setActive, sounds }) {
  ...
  return (
    <motion.nav
      ...
      animate={{
        width: isExpanded ? 'min(760px, calc(100vw - 24px))' : 18,
        height: isExpanded ? 'calc(var(--nav-h) - 12px)' : 18,
    ...
    >
      {!isExpanded ? (
        <motion.button
          ...
          transition={{ duration: 1.4, repeat: Infinity, repeatDelay: 0.9 }}
          style={{
            width: 18,
            height: 18,
            border: 'none',
            borderRadius: '50%',
            background: 'radial-gradient(circle at 35% 35%, rgba(255,255,255,0.95), var(--gold) 45%, rgba(200,169,110,0.35) 70%, rgba(200,169,110,0.1) 100%)',
            boxShadow: '0 0 0 1px rgba(200,169,110,0.15), 0 0 18px rgba(200,169,110,0.22)',
          }}
        />
      ) : (
        tabs.map((tab) => {
